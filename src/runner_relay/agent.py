...
class RelayClient:
    ...
    def _request(self, method: str, path: str, payload: dict) -> tuple[int, dict]:
        ...
        try:
            ...
            raise RelayError(f"relay request failed with HTTP {exc.code}: {detail}") from exc
        except urllib.error.URLError as exc:
            raise RelayError(f"relay request failed: {exc.reason}") from exc


class _LeaseHeartbeat(threading.Thread):
    def __init__(self, client: RelayClient, job_id: str, device: str, lease_seconds: int) -> None:
