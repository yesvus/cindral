...
from runner_relay.agent import Agent, JobSpec, RelayClient, RelayError
...
class RelayClientTest(unittest.TestCase):
    @patch("runner_relay.agent.urllib.request.urlopen")
    def test_claim_parses_the_job(self, urlopen) -> None:
        ...
        claimed = RelayClient("https://relay.example/", "token").claim("gurbet", ["arm64"])
    ...
    def test_claim_returns_none_when_the_queue_is_empty(self, urlopen) -> None:
        ...
        self.assertIsNone(RelayClient("https://relay.example", "token").claim("gurbet"))
    ...
    def test_http_errors_become_relay_errors(self, urlopen) -> None:
        ...
        with self.assertRaises(RelayError):
            RelayClient("https://relay.example", "token").claim("gurbet")

    @patch("runner_relay.agent.urllib.request.urlopen")
    def test_report_sends_the_exit_code(self, urlopen) -> None:
        urlopen.return_value = Response(json.dumps({"status": "failure"}).encode())
        payload = RelayClient("https://relay.example", "token").report("job-1", "gurbet", 7)
